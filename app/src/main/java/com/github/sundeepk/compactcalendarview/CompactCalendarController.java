...
class CompactCalendarController {
    ...
    private float headerHeightConst;
    ...
    private Rect dayRect = new Rect(0,0,widthPerDay, heightPerDay);
    ...
    private int balancePositiveColor;
    private int balanceNegativeColor;
    private int balanceDayColor;
    ...
    private HashMap<String, Double> dataSet;
    private HashMap<String, Double> dayBalances;
    private double balanceForToday;
    private double balanceForCurrentDate;

    Date getCurrentDate() {
        return currentDate;
    }

    void setDataSet(HashMap<String, Double> dataSet) {
        this.dataSet = dataSet;
    }

    HashMap<String, Double> getDataSet() {
        return dataSet;
    }

    public void transactionUpdate(Date dia) {
        //listener.onDayClick(dia);
        performOnDayClickCallback(dia);
    }

    public void setDayBalances(HashMap<String, Double> dayBalances) {
        this.dayBalances = dayBalances;
    }

    public void updateBalance(double value) {
        dayBalances.put(String.valueOf(currentCalender.get(Calendar.DATE)), value);
    }

    public void setBalanceForToday(double balanceForCurrentDate) {
        this.balanceForToday = balanceForCurrentDate;
    }

    public void setBalanceForCurrentDate(double balanceForCurrentDate) {
        this.balanceForCurrentDate = balanceForCurrentDate;
    }
    ...
    CompactCalendarController(Paint dayPaint, OverScroller scroller, Rect textSizeRect, AttributeSet attrs,
                              ...
                              Locale locale, TimeZone timeZone, HashMap<String, Double> dataSet) {
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            ...
            try {
                currentDayIndicatorColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarCurrentDayIndicatorColor, currentDayIndicatorColor);
                ...
                float dimension = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, textSize, context.getResources().getDisplayMetrics());
                textSize = typedArray.getDimensionPixelSize(R.styleable.CompactCalendarView_compactCalendarTextSize, (int) dimension);
                ...
                balanceDayColor = typedArray.getColor(R.styleable.CompactCalendarView_balance_day_text_color, otherMonthDaysTextColor);
                balancePositiveColor = typedArray.getColor(R.styleable.CompactCalendarView_balance_positive_text_color, Color.GREEN);
                balanceNegativeColor = typedArray.getColor(R.styleable.CompactCalendarView_balance_negative_color, Color.RED);
    ...
    /**
     * Call this to manually change to the next Month
     */
    void showNextMonth() {
    ...
    /**
     * Call this to manually change to the previous Month
     */
    ...
    /**
     * Used to set manually the timeZone and the Locale
     * @param timeZone
     * @param locale
     */
    ...
    void onMeasure(int width, int height, int paddingRight, int paddingLeft) {
        ...
        headerHeightConst =  paddingHeight + 5;
        heightPerDay = targetHeight > 0 ? targetHeight / 7 : ((int) ((height + headerHeightConst) / 7));
    ...
    void onSingleTapUp(MotionEvent e) {
        ...
        int dayRow = Math.round(((e.getY() /*- paddingHeight*/ + textHeight/6 -(headerHeightConst/2)) / heightPerDay));
        ...
        if (dayOfMonth < calendarWithFirstDayOfMonth.getActualMaximum(Calendar.DAY_OF_MONTH)
            ...
            if ((dayRow>0) && (dayOfMonth < 0) && displayOtherMonthDays) {
    ...
    private void drawMonth(Canvas canvas, Calendar monthToDrawCalender, int offset) {
        ...
        canvas.drawLine(accumulatedScrollOffset.x+offset, 0, accumulatedScrollOffset.x+offset, this.height, dayPaint);
        dayPaint.setStrokeWidth(0);
        dayPaint.setStyle(Paint.Style.FILL);

        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {

            if (dayRow == 7) {          //Si ya termino con los dias de esta semana
                dayRow = 0;             //Reiniciar con lunes
                if (dayColumn <= 6) {   // Y continuar con la siguiente semana
                    dayColumn++;            //////
                }
            }

            if (dayColumn == dayColumnNames.length) {   //Si es la ultimo dia de esta semana...
                break;                                  //Continuar con la siguiente semana
            }
            ...
            float startX = accumulatedScrollOffset.x + offset + paddingLeft;
            float startY= (dayRow-1)*heightPerDay  /*- textHeight/6*/ + headerHeightConst;
            float endX = accumulatedScrollOffset.x + offset - paddingRight + (widthPerDay*7);

            if (dayColumn==0) if (dayRow >= 1) {
                dayPaint.setColor(Color.DKGRAY);
                dayPaint.setAlpha(255);
                canvas.drawLine(startX, startY, endX, startY, dayPaint);
            }
            dayPaint.setTypeface(Typeface.create("Roboto",Typeface.NORMAL));
            if (dayRow == 0) {
                ...
                if (shouldDrawDaysHeader) {
                    ...
                    canvas.drawLine(startX, paddingHeight+10,accumulatedScrollOffset.x+offset+widthPerDay*7,paddingHeight+10,dayPaint);
                }
            } else {
                dayPaint.setTextAlign(Paint.Align.RIGHT);
                //Second row and on, let´s draw days
                int day = ((dayRow - 1) * 7 + dayColumn + 1) - firstDayOfMonth;
                boolean insideMonth = day <= monthToDrawCalender.getActualMaximum(Calendar.DAY_OF_MONTH) && day > 0;
                /** Mostrar dias anteriores*/ insideMonth = day <= 0;

                float xpositionStart = accumulatedScrollOffset.x + offset + paddingLeft + widthPerDay * dayColumn;
                float xpositionEnd = accumulatedScrollOffset.x + offset + paddingLeft + widthPerDay * (dayColumn + 1);
                float ypositionEnd = startY + heightPerDay;

                 if (((int) currentCalender.get(Calendar.DAY_OF_MONTH)) == day && isSameMonthAsCurrentCalendar && !isAnimatingWithExpose) {
                     drawDayRectIndicator(currentSelectedDayIndicatorStyle, canvas, xpositionStart, startY-dayPaint.getStrokeWidth(),
                             xpositionEnd-paddingLeft, ypositionEnd, currentSelectedDayBackgroundColor);
...
//                    drawDayRectIndicator(currentDayIndicatorStyle, canvas, xpositionStart, startY-dayPaint.getStrokeWidth(),
//                            xpositionEnd-paddingLeft, ypositionEnd , currentDayBackgroundColor);
                    drawDayRectIndicator(currentDayIndicatorStyle, canvas, xpositionStart, startY-dayPaint.getStrokeWidth(),
                            xpositionEnd-paddingLeft, startY+10 , currentDayIndicatorColor);

                    drawBalances(String.valueOf(balanceForToday), xpositionEnd-paddingRight, (startY+textHeight/3 + 10), canvas);
                    drawText(false, String.valueOf(day)/*"·"*/, xpositionEnd-paddingRight/*xPosition*/, (startY+textHeight/3 + 10)/*yPosition*/, canvas);
                    dayPaint.setStyle(Paint.Style.FILL);
                    dayPaint.setColor(calenderTextColor);
                    dayPaint.setTextAlign(Paint.Align.RIGHT);
                    canvas.drawText(String.valueOf(day)/*"·"*/, xpositionEnd-paddingRight/*xPosition*/, (startY+textHeight/3 + 10)/*yPosition*/, dayPaint);

                    continue;
                }
                if (insideMonth && displayOtherMonthDays) {
                    drawText(displayOtherMonthDays,String.valueOf(maximumPreviousMonthDay + day),
                            xpositionEnd - paddingRight, (startY + textHeight / 3 + 10), canvas);
                    // Display day month before
                }
                else if (day > maximumMonthDay && displayOtherMonthDays) {
                    // Display day month after
                    drawText(displayOtherMonthDays,String.valueOf(day - maximumMonthDay),
                            xpositionEnd - paddingRight, (startY + textHeight / 3 + 10), canvas);
                } else if (day <= maximumMonthDay && day > 0){
                    //canvas.drawText("100", xPosition, yPosition-(bigCircleIndicatorRadius/2),dayPaint);
//                    canvas.drawText(String.valueOf(day)/*"·"*/, xpositionEnd-paddingRight/*xPosition*/, (startY+textHeight/3 + 10)/*yPosition*/, dayPaint);
                    drawText(false, String.valueOf(day)/*"·"*/, xpositionEnd-paddingRight/*xPosition*/, (startY+textHeight/3 + 10)/*yPosition*/, canvas);
                    if(isSameMonthAsCurrentCalendar && day == currentCalender.get(Calendar.DAY_OF_MONTH)){
                        drawBalances(String.valueOf(balanceForCurrentDate), xpositionEnd-paddingRight, (startY+textHeight/3 + 10), canvas);
                    }
//                    canvas.drawText("hola", xPosition, yPosition+(bigCircleIndicatorRadius/2),dayPaint);
//                    canvas.drawText("253", xPosition, yPosition+(bigCircleIndicatorRadius/2),dayPaint);
                }
            }
        }
    }

    private void drawBalances(String value, float x, float y, Canvas canvas) {
        dayPaint.setStyle(Paint.Style.FILL);
        dayPaint.setTextAlign(Paint.Align.RIGHT);
        dayPaint.setFakeBoldText(true);
        if (Double.valueOf(value) >= 0 ) {
            dayPaint.setColor(balancePositiveColor);
        }else dayPaint.setColor(balanceNegativeColor);
        dayPaint.setTextSize(textSize/2);

        canvas.drawText(value, x, y+textSize+2, dayPaint);
        dayPaint.setFakeBoldText(false);
        dayPaint.setTextSize(textSize);
    }

    private void drawText(boolean isOtherMonthDay, String day, float x, float y, Canvas canvas) {
        dayPaint.setStyle(Paint.Style.FILL);
        dayPaint.setTextAlign(Paint.Align.RIGHT);
        if (isOtherMonthDay) dayPaint.setColor(otherMonthDaysTextColor);
        else {
            //dayPaint.setFakeBoldText(true);
            dayPaint.setColor(balanceDayColor);
            dayPaint.setTextSize(textSize/2);
            Double value = dayBalances.get(day);

            try {
                if (value.compareTo(0.0d) != 0){
                    String valueText = String.valueOf(value);
                    canvas.drawText(valueText, x, y+textSize/2+2, dayPaint);
                }
                dayPaint.setTextSize(textSize);
                dayPaint.setColor(calenderTextColor);
    ...
    private void drawDayCircleIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color, float circleScale) {
        ...
        if (color== currentDayBackgroundColor){
            dayPaint.setColor(color);
            canvas.drawRect(x-(widthPerDay/2),y-(heightPerDay/2),x+(widthPerDay/2),y-(heightPerDay/2)+4, dayPaint);
            return;
        }
    ...
    private void drawDayRectIndicator(int indicatorStyle, Canvas canvas, float x, float y, float xFinal, float yFinal, int color){
        dayPaint.setColor(color);
        if (indicatorStyle==currentDayIndicatorStyle) {
            canvas.drawRect(x,y,xFinal,yFinal,dayPaint);
        }
//        else {
//
//        }
...
//        canvas.drawCircle(x, y, radius, dayPaint);
        canvas.drawRect((x+paddingLeft-(widthPerDay/2)), (y+2-(heightPerDay/2)), x+(widthPerDay/2), y-4+(heightPerDay/2), dayPaint
